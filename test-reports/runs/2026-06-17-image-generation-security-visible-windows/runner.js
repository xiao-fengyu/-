    ...
    req.on('end', () => {
      ...
      if (req.method === 'POST' && req.url.endsWith('/chat/completions')) {
        res.end(JSON.stringify({
          choices: [{ message: { content: 'A clean product photo of a white ceramic mug centered on a light gray background, natural light, high detail.' } }],
        }))
        return
      }
      if (req.method === 'POST' && req.url.endsWith('/images/generations')) {
        ...
        try {
          ...
          count = Math.max(1, Math.min(2, Number(parsed.n || 2)))
        } catch {}
        res.end(JSON.stringify({
          created: Math.floor(Date.now() / 1000),
          data: Array.from({ length: count }, (_, index) => ({ b64_json: pngBase64(index % 2 ? 'green' : 'red') })),
        }))
        return
      }
      res.statusCode = 404
...
async function addImageProviderViaUi(page) {
  ...
  await fillModalForm(page, {
    '名': report.created.imageProviderName,
    'API': MOCK_BASE,
    'Key': 'mock-key-redacted',
    '模': 'mock-image-model',
    '图': '2',
  })
...
async function addLlmViaUi(page) {
  ...
  await clickPrimaryExtraButton(page, 0)
  await fillModalForm(page, {
    '名': report.created.llmProviderName,
    'API': MOCK_BASE,
    'Key': 'mock-key-redacted',
    '模': 'mock-text-model',
    'Temperature': '0.1',
...
async function main() {
  ...
  let api
  try {
    addCheck('target commit is recorded', COMMIT === 'a1bb1d5', { commit: COMMIT })
    ...
    await page.locator('.gmt-t').filter({ hasText: '自然' }).click({ force: true })
    ...
    await selectByFieldLabel(page, '生成', '2')
    await selectByFieldLabel(page, '提供', report.created.imageProviderName)
    ...
    await page.getByRole('button', { name: /生成/ }).last().click({ force: true })
    ...
    addCheck('generated images remain visible after return via history', returnedMatches.length >= 2, { returnedMatches })
