...
仓库已内置 Windows Runner 工作流：`.github/workflows/build-windows-exe.yml`

触发方式：

1. 推送影响打包的相关文件到 `main`
2. 或在 GitHub → Actions 页面手动运行 `build-windows-exe`
...
产出：
- GitHub Actions Artifact：`e-platform-windows-x64`
- 内含 `release/*.exe` NSIS 安装包

#### 方案 B：本地 Windows 机器构建
